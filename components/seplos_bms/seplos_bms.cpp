...
void SeplosBms::on_telemetry_data_(const std::vector<uint8_t> &data) {
  ...
  this->publish_state_(this->state_of_charge_sensor_, (float) (residual_capacity_sensor_ / battery_capacity_sensor_) * 100); // there is no direct state of charge it seems - must be calculated
